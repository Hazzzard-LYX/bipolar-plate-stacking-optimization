...
def draw_case(case, panels):
    ...
    fig.suptitle("Experiment 17 Abaqus CPRESS - %s" % case["label"])
...
def main():
    ...
    by_interface = os.path.join(OUT_DIR, "exp17_small_disp_contact_uniformity_by_interface.csv")
    with open(by_interface, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    summary_csv = os.path.join(OUT_DIR, "exp17_small_disp_contact_uniformity_summary.csv")
    ...
    fig.suptitle("Experiment 17 Abaqus real surface-to-surface contact uniformity comparison")
    fig.tight_layout()
    comparison_png = os.path.join(OUT_DIR, "exp17_small_disp_contact_uniformity_comparison.png")
