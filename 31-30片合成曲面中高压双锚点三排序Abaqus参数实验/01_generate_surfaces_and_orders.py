...
def main():
    ...
    min_path, min_score = optimized_greedy_path(matrix, maximize=False)
    max_path, max_score = optimized_greedy_path(matrix, maximize=True)
    natural_path = list(range(len(surfaces)))
    natural_score = path_score(natural_path, matrix)
    natural_min = improve_two_opt(natural_path, matrix, maximize=False)
    natural_max = improve_two_opt(natural_path, matrix, maximize=True)
    if path_score(natural_min, matrix) < min_score:
        min_path, min_score = natural_min, path_score(natural_min, matrix)
    if path_score(natural_max, matrix) > max_score:
        max_path, max_score = natural_max, path_score(natural_max, matrix)
