...
foreach ($Case in @("min", "natural", "max")) {
    ...
    & $AbaqusLauncher python (Join-Path $ScriptDir "05_extract_displacement.py") $Job $Case 17 9
    if ($LASTEXITCODE -ne 0) { throw "Displacement extraction failed: $Job" }
}
& $PythonExe (Join-Path $ScriptDir "06_compare_results.py")
