...
CASES = [
    {
        ...
        "job": "exp20_min_pressure_midsoft_anchor_contact",
        "order": [1, 10, 9, 7, 11, 8, 12, 4, 14, 3, 5, 13, 15, 6, 2],
    },
    {
        "case": "natural",
        "label": "Natural",
        "job": "exp20_natural_pressure_midsoft_anchor_contact",
        "order": list(range(1, 16)),
    },
    {
        "case": "max_distance_sum",
        "label": "Max-distance-sum",
        "job": "exp20_max_pressure_midsoft_anchor_contact",
...
def draw_case(case, panels):
    ...
    fig.suptitle("Experiment 20 Abaqus CPRESS - %s" % case["label"])
...
def main():
    ...
    by_interface = os.path.join(OUT_DIR, "exp20_pressure_midsoft_anchor_contact_uniformity_by_interface.csv")
    with open(by_interface, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    summary_csv = os.path.join(OUT_DIR, "exp20_pressure_midsoft_anchor_contact_uniformity_summary.csv")
    ...
    fig.suptitle("Experiment 20 Abaqus real surface-to-surface contact uniformity comparison")
    fig.tight_layout()
    comparison_png = os.path.join(OUT_DIR, "exp20_pressure_midsoft_anchor_contact_uniformity_comparison.png")
