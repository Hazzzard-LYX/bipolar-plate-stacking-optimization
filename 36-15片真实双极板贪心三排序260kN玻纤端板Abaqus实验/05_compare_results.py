...
CPRESS_SUFFIX = os.environ.get("EXP36_CPRESS_SUFFIX", "")
...
def load_case(case):
    path = os.path.join(OUTPUT_DIR, job_name(case) + CPRESS_SUFFIX + "_cpress_nodes.csv")
    panels = {}
    all_node_values = []
...
def read_endplate_response(case):
    path = os.path.join(OUTPUT_DIR, job_name(case) + "_endplate_response.csv")
    with open(path, "r", newline="", encoding="utf-8-sig") as handle:
...
def summarize(case):
    ...
    endplate = read_endplate_response(case)
    return {
        "case": case,
        "label": LABELS[case],
        ...
        "top_endplate_u3_range_mm": 1000.0 * float(endplate["top_u3_range_m"]),
        "top_endplate_max_abs_u3_mm": 1000.0 * float(endplate["top_max_abs_u3_m"]),
        "top_endplate_max_mises_mpa": float(endplate["top_max_mises_pa"]) / 1.0e6,
        "bottom_endplate_max_mises_mpa": float(endplate["bottom_max_mises_pa"]) / 1.0e6,
        "bottom_reaction_z_kn": float(endplate["bottom_reaction_z_n"]) / 1000.0,
        "bottom_reaction_tail_mean_kn": float(endplate["bottom_reaction_tail_mean_n"]) / 1000.0,
        "bottom_reaction_tail_min_kn": float(endplate["bottom_reaction_tail_min_n"]) / 1000.0,
        "bottom_reaction_tail_max_kn": float(endplate["bottom_reaction_tail_max_n"]) / 1000.0,
    }


def main():
    rows = [summarize(case) for case in CASES]
    summary_path = os.path.join(OUTPUT_DIR, "exp%d_uniformity_summary%s.csv" % (cfg.EXPERIMENT_ID, CPRESS_SUFFIX))
    ...
    fig.tight_layout()
    png_path = os.path.join(OUTPUT_DIR, "exp%d_uniformity_comparison%s.png" % (cfg.EXPERIMENT_ID, CPRESS_SUFFIX))
