...
CASES = [
    {
        ...
        "job": "exp25_min_explicit_held_pressure0448_contact",
        "order": [1, 10, 9, 7, 11, 8, 12, 4, 14, 3, 5, 13, 15, 6, 2],
    },
    {
        "case": "natural",
        "label": "Natural",
        "job": "exp25_natural_explicit_held_pressure0448_contact",
        "order": list(range(1, 16)),
    },
    {
        "case": "max_distance_sum",
        "label": "Max-distance-sum",
        "job": "exp25_max_explicit_held_pressure0448_contact",
...
def draw_case(case, panels):
    ...
    fig.suptitle("Experiment 25 Abaqus Explicit CPRESS - %s" % case["label"])
...
def main():
    ...
    by_interface = os.path.join(OUT_DIR, "exp25_pressure0448_explicit_uniformity_by_interface.csv")
    with open(by_interface, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    summary_csv = os.path.join(OUT_DIR, "exp25_pressure0448_explicit_uniformity_summary.csv")
    ...
    fig.suptitle("Experiment 25 Abaqus Explicit quasi-static contact uniformity comparison")
    fig.tight_layout()
    comparison_png = os.path.join(OUT_DIR, "exp25_pressure0448_explicit_uniformity_comparison.png")
