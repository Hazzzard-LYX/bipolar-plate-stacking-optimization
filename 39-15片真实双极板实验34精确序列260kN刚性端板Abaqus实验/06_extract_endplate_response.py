...
for value in frame.fieldOutputs["U"].values:
    ...
    if in_range(label, TOP_NODE_MIN, TOP_NODE_MAX) or label == TOP_RP:
        top_u3.append(float(value.data[2]))
    elif in_range(label, BOTTOM_NODE_MIN, BOTTOM_NODE_MAX) or label == BOTTOM_RP:
        bottom_u3.append(float(value.data[2]))

bottom_reaction_z = 0.0
for value in frame.fieldOutputs["RF"].values:
    if int(value.nodeLabel) == BOTTOM_RP:
