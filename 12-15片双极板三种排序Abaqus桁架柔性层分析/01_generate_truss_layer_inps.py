...
CASES = [
    ("min_distance_sum", "exp12_min_truss_layer", [1, 10, 9, 7, 11, 8, 12, 4, 14, 3, 5, 13, 15, 6, 2]),
    ("natural", "exp12_natural_truss_layer", list(range(1, 16))),
    ("max_distance_sum", "exp12_max_truss_layer", [11, 12, 13, 7, 14, 9, 5, 6, 8, 15, 3, 1, 2, 10, 4]),
]
...
def main():
    ...
    for case_name, job, order in CASES:
        ...
        map_path = os.path.join(OUTPUT_DIR, job + "_truss_map.csv")
        nodes = []
        elems = []
        spring_rows = []
        upper_nodes = []
        ...
        for iface in range(1, len(order)):
            ...
            for j, y in enumerate(ys):
                for i, x in enumerate(xs):
                    lower = node_id
                    upper = node_id + 1
                    area = float(k_grid[j, i]) * TRUSS_LENGTH_M / TRUSS_E_PA
                    nodes.append((lower, x, y, z))
                    nodes.append((upper, x, y, z + TRUSS_LENGTH_M))
                    lower_nodes.append(lower)
                    upper_nodes.append(upper)
                    elems.append((elem_id, lower, upper, float(k_grid[j, i]), area))
                    spring_rows.append({
                        "case": case_name,
                        "job": job,
        ...
        with open(inp_path, "w") as f:
            f.write("*Heading\n")
            f.write("Experiment 12 - Abaqus T3D2 equivalent compliant layer: %s\n" % case_name)
            ...
            f.write("*Material, name=TRUSS_MAT\n")
            f.write("*Elastic\n%.10e, 0.0\n" % TRUSS_E_PA)
            for eid, n1, n2, k, area in elems:
                f.write("*Element, type=T3D2, elset=E%d\n" % eid)
                f.write("%d, %d, %d\n" % (eid, n1, n2))
                f.write("*Solid Section, elset=E%d, material=TRUSS_MAT\n%.10e,\n" % (eid, area))
            f.write("*Boundary\n")
