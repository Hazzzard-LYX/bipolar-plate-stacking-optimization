...
CASES = [
    ("min_distance_sum", "exp10_min_spring_layer", [1, 10, 9, 7, 11, 8, 12, 4, 14, 3, 5, 13, 15, 6, 2]),
    ("natural", "exp10_natural_spring_layer", list(range(1, 16))),
    ("max_distance_sum", "exp10_max_spring_layer", [11, 12, 13, 7, 14, 9, 5, 6, 8, 15, 3, 1, 2, 10, 4]),
]
...
def main():
    ...
    for case_name, job, order in CASES:
        ...
        map_path = os.path.join(OUTPUT_DIR, job + "_spring_map.csv")
        nodes = []
        elems = []
        spring_rows = []
        node_id = 1
        ...
        for iface in range(1, len(order)):
            ...
            for j, y in enumerate(ys):
                for i, x in enumerate(xs):
                    nodes.append((node_id, x, y, -0.002 * iface))
                    elems.append((elem_id, node_id, float(k_grid[j, i])))
                    spring_rows.append({
                        "case": case_name,
                        "job": job,
        ...
        with open(inp_path, "w") as f:
            f.write("*Heading\n")
            f.write("Experiment 10 - Abaqus equivalent compliant spring layer: %s\n" % case_name)
            f.write("*Preprint, echo=NO, model=NO, history=NO, contact=NO\n")
            f.write("** Stack order: %s\n" % " -> ".join(str(x) for x in order))
            f.write("*Node\n")
            for row in nodes:
                f.write("%d, %.10e, %.10e, %.10e\n" % row)
            f.write("*Nset, nset=ALL_PRESSURE_NODES\n")
            write_list(f, [n[0] for n in nodes])
            for eid, nid, k in elems:
                f.write("*Element, type=SPRING1, elset=E%d\n" % eid)
                f.write("%d, %d\n" % (eid, nid))
                f.write("*Spring, elset=E%d\n3\n%.10e\n" % (eid, k))
            f.write("*Boundary\n")
            f.write("ALL_PRESSURE_NODES, 1, 2, 0.0\n")
            f.write("*Step, name=Compression, nlgeom=NO\n")
            f.write("*Static\n1.0, 1.0\n")
            f.write("*Boundary\n")
            f.write("ALL_PRESSURE_NODES, 3, 3, %.10e\n" % PRESCRIBED_U3_M)
