...
CASES = [
    {
        ...
        "job": "exp34_exact_min_explicit_smallslip_friction_contact",
        "order": [2, 14, 3, 5, 4, 12, 11, 8, 13, 1, 10, 7, 9, 15, 6],
    },
    {
        "case": "natural",
        "label": "Natural",
        "job": "exp34_natural_explicit_smallslip_friction_contact",
        "order": list(range(1, 16)),
    },
    {
        "case": "max_distance_sum",
        "label": "Max-distance-sum",
        "job": "exp34_exact_max_explicit_smallslip_friction_contact",
        "order": [11, 1, 2, 10, 14, 9, 13, 7, 5, 6, 3, 15, 8, 4, 12],
    },
]
...
def read_node_values(case):
    path = os.path.join(OUT_DIR, case["job"] + "_cpress_nodes.csv")
    with open(path, "r", newline="", encoding="utf-8-sig") as f:
        return np.array([max(float(row["cpress_pa"]), 0.0) for row in csv.DictReader(f)], dtype=float)
...
def draw_case(case, panels):
    ...
    fig.suptitle("Experiment 34 exact-DP Abaqus Explicit frictional CPRESS - %s" % case["label"])
...
def main():
    ...
    for case in CASES:
        ...
        node_values = read_node_values(case)
        ...
        item["positive_node_coverage"] = float(np.mean(node_values > 0.0))
        item["absolute_peak_mpa"] = float(np.max(node_values) / 1.0e6)
        summary.append(item)

    by_interface = os.path.join(OUT_DIR, "exp34_exact_pressure0448_friction_uniformity_by_interface.csv")
    with open(by_interface, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    summary_csv = os.path.join(OUT_DIR, "exp34_exact_pressure0448_friction_uniformity_summary.csv")
    ...
    fig.suptitle("Experiment 34 exact-DP Abaqus Explicit small-sliding friction uniformity comparison")
    fig.tight_layout()
    comparison_png = os.path.join(OUT_DIR, "exp34_exact_pressure0448_friction_uniformity_comparison.png")
