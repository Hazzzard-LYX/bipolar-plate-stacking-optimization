...
JOB_NAME = "exp34_exact_max_explicit_smallslip_friction_contact"
INP_PATH = os.path.join(OUTPUT_DIR, JOB_NAME + ".inp")

STACK_ORDER = [11, 1, 2, 10, 14, 9, 13, 7, 5, 6, 3, 15, 8, 4, 12]
LENGTH_M = 0.820
WIDTH_M = 0.345
RAW_NX = 201
RAW_NY = 85
...
TOP_ENDPLATE_MASS_KG = LENGTH_M * WIDTH_M * ENDPLATE_THICKNESS_M * ENDPLATE_DENSITY
...
def find_data_dir():
    for root, _dirs, files in os.walk(PROJECT_ROOT):
        csv_names = [n for n in files if n.lower().endswith(".csv")]
        if "真实双极板曲面数据_820x345mm" in root and len(csv_names) >= 15:
            return root
    raise RuntimeError("Cannot find the measured 820x345mm CSV data directory under %s" % PROJECT_ROOT)
...
def write_inp():
    ...
    with open(INP_PATH, "w") as f:
        f.write("*Heading\n")
        f.write("Experiment 34 - exact max-distance-sum small-sliding friction Explicit contact model\n")
        ...
        for i, plate in enumerate(plates):
            f.write("*Element, type=S4, elset=P%02d_EALL\n" % (i + 1))
            for row in plate["elems"]:
                f.write("%d, %d, %d, %d, %d\n" % row)
        f.write("*Element, type=R3D4, elset=EP_BOTTOM_EALL\n")
        for row in bottom_end["elems"]:
            f.write("%d, %d, %d, %d, %d\n" % row)
        f.write("*Element, type=MASS, elset=EP_TOP_MASS\n")
        f.write("9400001, %d\n" % TOP_ENDPLATE_RP)
        f.write("*Mass, elset=EP_TOP_MASS\n%.10e\n" % TOP_ENDPLATE_MASS_KG)
        ...
        f.write("*Contact Damping, definition=DAMPING COEFFICIENT\n%.10e\n" % CONTACT_DAMPING_PA_S_PER_M)
        ...
        f.write("*Amplitude, name=SMOOTH_PRESSURE, definition=SMOOTH STEP\n")
        f.write("0.0, 0.0, %.10e, 1.0, %.10e, 1.0\n" % (RAMP_TIME_S, STEP_TIME_S))
        f.write("*Step, name=Compression, nlgeom=YES\n")
        ...
        f.write("*Cload, amplitude=SMOOTH_PRESSURE\n")
        f.write("EP_TOP_RP, 3, %.10e\n" % (-TOP_LOAD_N))
        f.write("*Dload, amplitude=SMOOTH_PRESSURE\n")
        for i in range(len(plates)):
        ...
        f.write("*Output, history, time interval=1.0e-2\n")
        f.write("*Energy Output\n")
        f.write("ALLIE, ALLKE, ALLAE, ALLVD, ALLWK\n")
        f.write("*End Step\n")
