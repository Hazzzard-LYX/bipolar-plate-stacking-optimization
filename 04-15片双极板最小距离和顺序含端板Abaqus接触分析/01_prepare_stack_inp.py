...
JOB_NAME = "exp4_stack15_min_with_endplates"
...
def write_inp():
    ...
    top_end = build_rect_mesh("EP_TOP", 9100000, 9100000, top_z + 0.5 * ENDPLATE_THICKNESS_M)
    bottom_end = build_rect_mesh("EP_BOTTOM", 9200000, 9200000, bottom_z - 0.5 * ENDPLATE_THICKNESS_M)
    ...
    with open(INP_PATH, "w") as f:
        f.write("*Heading\n")
        f.write("Experiment 4 - min-distance stack with top/bottom endplates\n")
        ...
        f.write("P01_CONTACT_NODES, EP_TOP_BOTTOM\n")
        for i in range(len(plates) - 1):
            f.write("*Contact Pair, interaction=LINEAR_FRICTIONLESS, type=NODE TO SURFACE, small sliding\n")
            f.write("P%02d_CONTACT_NODES, P%02d_TOP\n" % (i + 1, i + 2))
        f.write("*Contact Pair, interaction=LINEAR_FRICTIONLESS, type=NODE TO SURFACE, small sliding\n")
        f.write("P%02d_CONTACT_NODES, EP_BOTTOM_TOP\n" % len(plates))
        ...
        f.write("*Static, stabilize=2.0e-3\n1.0e-4, 1.0, 1.0e-14, 1.0e-4\n")
        f.write("*Cload\n")
    ...
    print("Rigid endplate reference load: %.6e N" % -TOP_LOAD_N)
